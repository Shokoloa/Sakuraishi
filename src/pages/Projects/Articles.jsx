        ...
        <div className="project-card_line">
            {data.map((project, index) => {
                ...
                if (project.status === "op") {
                    statusColor = '#22c55e';
                    statusText = t('projects.online');
                } else if (project.status === "up") {
                    statusColor = '#0096c7';
                    statusText = t('projects.update');
                } else if (project.status === "in") {
                    statusColor = '#f4a261';
                    statusText = t('projects.inDev');
                } else {
                    statusColor = '#ef4444';
                    statusText = t('projects.offline');
                }
                ...
                return (
                    <article key={index} className="project-card" style={{ '--propertyColor': statusColor }} onClick={() => project.url ? window.open(project.url) : null}>
                        <div className="content">
                            ...
                            <p>{project.desc?.fr ? i18n.language === 'en' ? project.desc.en : project.desc.fr : project.desc}</p>
                        </div>
